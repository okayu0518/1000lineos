...
__attribute__((naked)) void switch_context(uint32_t *prev_sp,
    ...
    // save registers to a stack of running process
    "addi sp, sp, -13 * 4\n"
    "sw ra, 0 * 4(sp)\n"
    "sw s0, 1 * 4(sp)\n"
    "sw s1, 2 * 4(sp)\n"
    ...
    "sw sp, (a0)\n"
    "lw sp, (a1)\n"
    // restore registers from a stack of running process
    "lw ra, 0 * 4(sp)\n"
    "lw s0, 1 * 4(sp)\n"
    "lw s1, 2 * 4(sp)\n"
    "lw s2, 3 * 4(sp)\n"
...
__attribute__((naked)) void user_entry(void) {
    __asm__ __volatile__(
        "csrw sepc, %[sepc]\n"       // set program counter to USER_BASE
...
struct process *create_process(const void *image, size_t image_size) {
    ...
    // stack registers to get back with switch_context()
    ...
    *--sp = (uint32_t) user_entry; // ra
    ...
    // map MMIO for virtio block device
...
void fs_init(void) {
    for (unsigned sector = 0; sector < sizeof(disk) / SECTOR_SIZE; sector++)
        // read disk image to memory
        // スタックの大きさに限りがあるため，diskは静的変数にする
    ...
    switch (f->a3) {
        ...
        case SYS_WRITEFILE: {
            ...
            if (len > (int) sizeof(file->data))
                len = file->size; // limit to file size

            if (f->a3 == SYS_WRITEFILE) {
                memcpy(file->data, buf, len);
                file->size = len;
                fs_flush(); // flush changes to disk
            } else{ // SYS_READFILE
                memcpy(buf, file->data, len);
            }

            f->a0 = len; // return number of bytes read/written
            break;
        }
        default:
...
void handle_trap(struct trap_frame *f) {
    ...
    if (scause == SCAUSE_ECALL) {
        handle_syscall(f);
        user_pc += 4;    // increment program counter when returning to user-mode to next instruction
    } else {
